...
/*You can create and configure a GLKView , Before you can use GLKView for drawing, you must associate it with an EAGLContext object
 ...
 */
  ...
  [self setupGL];
    ...
    //change between finger and motion
  ...
  if(_isUsingMotion) {
    ...
    if (d != nil) {
      ...
      if (orientation == UIDeviceOrientationLandscapeLeft ){
        cPitch = cPitch*-1; // correct depth when in landscape left
      }
...
-(void)renderInHTY:(CVPixelBufferRef)pb{
    ...
    CVPixelBufferRef pixelBuffer = pb;//[self.videoPlayerController retrievePixelBufferToDraw];
